...
面向产品经理的**面试后复盘与长期训练**工具。它不做实时面试辅助，而是帮你在
...
- **录音转写**：上传面试录音（MP3/M4A/WAV/AAC/OGG/WebM，≤50MB），Gemini 转成带
  时间戳的文字。原始音频和简历文件只写入受控的本机临时目录，处理后删除；应用启动时清理上次异常中断遗留的本程序临时文件，不进入长期数据 store。
...
### 面经资料库（可溯源）
- **联网发现**：用 Google Search 发现候选公开面经（牛客、小红书等），自动尝试读取白名单公开 HTML 并回填元数据；动态脚本壳、访问受限或正文不足时保留原帖入口和人工确认状态。
- **证据门槛**：自动读取或你录入原帖正文摘录后，AI 预审分为 `AI 预审通过 · 待确认` / `待人工确认` / `不使用`，自动读取仍标记为未人工确认。
  - `auto_approved` 只能生成问题线索；只有你人工确认后的 `approved` 资料，才会进入后续事实型复盘引用。
  - 评论只作为可信度信号，不作为事实证明。
  - 所有资料都保留原链接与发布日期。
...
见 [DEPLOY_RENDER.md](../../DEPLOY_RENDER.md)。仓库自带的 `render.yaml` 是**免费层**配置
（单实例、无持久盘、`APP_DEMO_MODE=1` 只读演示），适合部署一个公开可点的演示；如需保存
真实数据，参考该文档切换到付费层并挂持久盘。**部署到公网必须设置 `APP_ACCESS_TOKEN`**。
...
（agent loop / 工具 / 权限 / 日志）。当前的复盘助手已从中抽出、解耦为独立的 `app/`，
老版保留作参考。相关早期设计文档见 [../legacy/](../legacy/) 目录下的
`MINI_AGENT_BUILD_PLAN.md`、`OPENCLAW_EXPERIMENT_PLAYBOOK.md` 等。
