...
const DEMO = (() => {
  ...
  return { interview, interviewSummary, resume, resumeSummary, research, memory, skills, noteQuestions, agentResult, jdAnalysis, ...overrides };
})();
...
function renderJdResearch(data) { const root = $('#jd-research-handoff'); const candidates = data?.candidates || []; root.hidden = !candidates.length; if (!candidates.length) return; root.querySelector('strong').textContent = `已按 JD 自动发现 ${candidates.length} 条面经线索`; root.querySelector('span').textContent = '静态演示已自动识别标题、原帖链接、平台、轮次和搜索词；正文仍需原帖可见内容才能作为证据。'; }
...
function renderAgentCollected(candidates) { const root = $('#discovery-results'); root.replaceChildren(); if (!candidates.length) { root.innerHTML = '<p class="empty-copy">Agent 未收集到通过来源校验的候选。</p>'; return; } candidates.forEach(candidate => { const card = document.createElement('article'); card.className = 'discovery-item'; card.innerHTML = '<a target="_blank" rel="noreferrer"></a><p></p><div><span></span><button class="secondary-button compact-button" type="button">带入资料库</button></div>'; card.querySelector('a').href = candidate.url; card.querySelector('a').textContent = candidate.title; const screening = candidate.screening || {}; card.querySelector('p').textContent = `${candidate.summary || '候选公开资料'}${screening.reason ? ` · 初筛：${screening.reason}` : ''}`; card.querySelector('span').textContent = [candidate.platform, candidate.platform_id ? `来源 ${candidate.platform_id}` : '', screening.relevance != null ? `相关度提示 ${screening.relevance}/100` : '', candidate.provenance_status || '需人工确认'].filter(Boolean).join(' · '); card.querySelector('button').onclick = demoWrite; root.append(card); }); }
...
$('#research-new').onclick = resetResearch; $('#research-form').onsubmit = event => { event.preventDefault(); demoWrite(); }; $('#discover-form').onsubmit = discoverResearch; $('#agent-button').onclick = runResearchAgent; $('#generate-growth').onclick = demoWrite;
