"""Local web application for the Autumn Recruiting Assistant V3."""
...
from agent_runtime.cli import build_runtime
from audio_transcription import AudioTranscriptionError, extract_resume_file, transcribe_audio
from growth_memory import build_candidate_memory
from interview_review import extract_job_description, generate_growth_report, generate_interview_review, sample_interview
from interview_store import InterviewStore
from pm_skills import public_skills
from research_grounding import ResearchGroundingError, assess_public_source, discover_public_sources
...
class AutumnAssistantHandler(BaseHTTPRequestHandler):
    model_name = "gemini"
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/health":
            try:
                runtime = build_agent(self.model_name)
                self.send_json(
                    {
                        "ok": True,
                        "provider": self.model_name,
                        "model": getattr(runtime.model, "model", self.model_name),
                        "active_provider": getattr(runtime.model, "active_provider", self.model_name),
                        "providers": getattr(runtime.model, "provider_names", [self.model_name]),
                    }
                )
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if path == "/api/growth-report":
            ...
            try:
                ...
                report = generate_growth_report(runtime.model, records, memory)
        ...
        if path == "/api/extract-jd":
            ...
            try:
                runtime = build_agent(self.model_name)
                self.send_json({"ok": True, "analysis": extract_job_description(runtime.model, raw_jd)})
            except Exception:
        ...
        if len(parts) == 4 and parts[:2] == ["api", "research"] and parts[3] == "assess":
            ...
            try:
                runtime = build_agent(self.model_name)
                assessment = assess_public_source(runtime.model, record, RESEARCH_STORE.list())
    ...
    def handle_audio_upload(self) -> None:
        ...
        if not 0 < content_length <= MAX_AUDIO_BYTES:
            self.send_json({"ok": False, "error": "音频文件需小于 50 MB。请裁剪为本轮面试的相关片段后重试。"}, 400)
            return
        if "multipart/form-data" not in self.headers.get("Content-Type", ""):
            self.send_json({"ok": False, "error": "音频上传格式不正确。"}, 400)
            return
        ...
        if not form.getvalue("consent") == "true" or "audio" not in form:
            self.send_json({"ok": False, "error": "上传前请确认你有权使用这段录音。"}, 400)
            return
        item = form["audio"]
        filename = os.path.basename(str(getattr(item, "filename", "audio")))
        mime_type = str(getattr(item, "type", "") or mimetypes.guess_type(filename)[0] or "")
        ...
        try:
            with tempfile.NamedTemporaryFile(prefix="autumn-audio-", suffix=suffix, delete=False) as handle:
                ...
                while True:
                    ...
                    handle.write(chunk)
            runtime = build_agent(self.model_name)
            transcription = transcribe_audio(runtime.model, temporary_path, mime_type)
    ...
    def handle_resume_file_upload(self) -> None:
        try:
            ...
            content_length = 0
        if not 0 < content_length <= MAX_RESUME_FILE_BYTES:
            self.send_json({"ok": False, "error": "简历文件需小于 25 MB。"}, 400)
            return
        if "multipart/form-data" not in self.headers.get("Content-Type", ""):
            self.send_json({"ok": False, "error": "简历上传格式不正确。"}, 400)
            return
        form = cgi.FieldStorage(
            fp=self.rfile,
            headers=self.headers,
        ...
        if form.getvalue("consent") != "true" or "resume" not in form:
            self.send_json({"ok": False, "error": "解析前请确认你同意将文件发送给 Gemini。"}, 400)
            return
        item = form["resume"]
        filename = os.path.basename(str(getattr(item, "filename", "resume")))
        mime_type = str(getattr(item, "type", "") or mimetypes.guess_type(filename)[0] or "")
        ...
        try:
            with tempfile.NamedTemporaryFile(prefix="autumn-resume-", suffix=os.path.splitext(filename)[1], delete=False) as handle:
                ...
                while True:
                    ...
                    handle.write(chunk)
            runtime = build_agent(self.model_name)
            self.send_json({"ok": True, "resume": extract_resume_file(runtime.model, temporary_path, mime_type)})
        except AudioTranscriptionError as exc:
            self.send_json({"ok": False, "error": str(exc)}, 502)
        except Exception:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Autumn Recruiting Assistant V3")
    parser.add_argument("--port", type=int, default=int(os.environ.get("PORT", "8765")))
    parser.add_argument("--host", default=os.environ.get("HOST", "127.0.0.1"))
    parser.add_argument("--model", choices=["gemini", "mock", "openai"], default="gemini")
    args = parser.parse_args()
    AutumnAssistantHandler.model_name = args.model
    server = ThreadingHTTPServer((args.host, args.port), AutumnAssistantHandler)
    print("Autumn Recruiting Assistant running at http://%s:%s" % (args.host, args.port))
    print("Provider: %s. Press Ctrl-C to stop." % args.model)
    try:
