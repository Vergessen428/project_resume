...
用 `static_demo/` 这个纯前端目录，传到 HF **免费 Static Space**。访客打开就能看到“诊断总结 → 短期训练 → 长期记忆”的完整路径；写操作（保存/AI/上传）会提示"需在完整版体验"，公开搜索按钮展示静态 Agent 查询轨迹、公开页读取尝试和人工确认状态。
...
静态版不会休眠、秒开、永远免费。静态候选明确是合成演示数据，不对应真实小红书原帖；真实后端才会尝试读取限定平台的公开 HTML，遇到脚本壳或访问限制会保留待确认。缺点：没有实时 AI 生成（但演示看的就是做好的诊断，够用）。
