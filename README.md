...
| 能力 | 状态 | 边界 |
| --- | --- | --- |
| JD 提取 / AI 复盘 / 阶段报告 | ✅ 真实可用 | 纯文本调用，配好任一供应商 Key 即可 |
| 音频转写 / 简历解析 | ✅ 真实实现 | 依赖 Gemini Files API 配额 |
| **联网资料发现（Google Search）** | ⚠️ 受配额限制 | Gemini 的 Search grounding 是**独立且极紧的配额**，免费层几乎必然返回 `429`；此时可先**手动录入原帖**再走 AI 预审，或更换有配额的 Key |

> 复盘的每条 `evidence` 都经过后端校验：只有能在转写原文里定位到的引用才会展示，模型自己的
> 判断性描述不会冒充「原文证据」。这是「证据优先」原则的硬约束。
