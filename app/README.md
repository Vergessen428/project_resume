...
- **面经资料库**：Google Search 发现候选公开资料；录入原帖正文后由 AI 分为
  `AI 预审通过 · 待确认` / `待人工确认` / `不使用`。AI 结果只生成出题线索，复盘事实引用只允许人工确认后的资料，并保留原链接与日期。
- **PM 技能包**：产品判断、项目主导力、指标与实验、推进与协作、结构化表达、业务与岗位理解。
- **长短期记忆**：单场复盘保留当前上下文；阶段报告从历次已复盘面试中提取重复薄弱点、
  行动项和能力变化。
- **面后结果反馈**：可在面试档案中回填“等待结果 / 通过 / 未通过”，仅作为自报训练信号进入长期记忆，
  不改变本场评分，也不用于推断录用概率；样本不足时会明确降级。
...
本地回环地址不设访问口令时直接进入工作台；设置 `APP_ACCESS_TOKEN` 后需在登录框输入相同口令。
绑定到 `0.0.0.0` 或其他非回环地址时，非 Demo 模式必须设置 `APP_ACCESS_TOKEN`，否则服务会拒绝启动。
